...
public class PredictThanosSnap {
    ...
    public static void main (String[] args) {
    	...
    	String predictInput = args[0];
        String predictOutput = args[1];
        StdIn.setFile(predictInput);
        StdRandom.setSeed(StdIn.readLong());
        int vertices = StdIn.readInt();
        int[][]matrix = new int[vertices][vertices];
        ArrayList<Integer> deletedNodes = new ArrayList<Integer>();
        StdOut.setFile(predictOutput);
        ...
        {
            if(visited[i]==false && !deletedNodes.contains(i))
            {
                connect = false;
                break;
            }
    ...
    private static void dfs(int s, int[][] matrix, boolean[] visited)
    {
        visited[s] = true;
        for(int i = s; i<s+1; i++)
        {
