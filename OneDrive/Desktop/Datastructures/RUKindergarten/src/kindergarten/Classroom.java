...
public class Classroom {
    ...
    public void enterClassroom ( String filename ) {
        ...
        int h = StdIn.readInt();
        Student studentHead = new Student(first, last, h); 
        SNode firstStudent = new SNode(studentHead, null); //Sets the Node
        firstStudent.setStudent(studentHead); // Sets data of the node
        studentsInLine = firstStudent;
        studentsInLine.setStudent(studentHead); //Sets the data of the Node beacuse it is the first node in the list
        ...
        for(int i = 1; i<numStudents; i++)
        {
            ...
            SNode newStudent = new SNode(student, null); //Sets the node
            newStudent.setNext(studentsInLine);
            newStudent.setStudent(student);
            studentsInLine = newStudent;
            studentsInLine.setStudent(student); //Sets the data of the node
        }
    ...
    public void setupSeats(String seatingChart) {
        ...
        {
            for(int col = 0; col<seatingLocation[row].length; col++)
            {
                seatingLocation[row][col] = StdIn.readBoolean();
            }
        }
    ...
    public void seatStudents () {
        ...
        for(int row = 0; row<seatingLocation.length; row++)
        {
            for(int col = 0; col<seatingLocation[row].length; col++)
            {
                if(seatingLocation[row][col]==true && studentsInLine!=null)
                {
    ...
    public void insertMusicalChairs () {
        ...
        for(int row = 0; row<studentsSitting.length; row++)
        {
            ...
            {
                if(studentsSitting[row][col]!=null && firstCheck)
                {
                ...
                else if(studentsSitting[row][col]!=null)
                {
                    SNode newStudent = new SNode(studentsSitting[row][col], null);
    ...
    { 
        ...
        if(rand==0)
        {
            Student stud = ptr.getNext().getStudent();
    ...
    public void insertByHeight(Student studentToInsert) 
    {
        ...
        else if(studentsInLine.getNext()==null && studentsInLine.getStudent().getHeight()<=studentToInsert.getHeight()) //Second student inserted
        {
            studentsInLine.setNext(student);
        }
        else if(studentsInLine.getStudent().getHeight()>studentToInsert.getHeight()) //Shortest student inserted first in line
        {
        ...
        else
        {
            ...
            while(!found)
            {
                if(ptr.getNext()==null && ptr.getStudent().getHeight()<=studentToInsert.getHeight())
                {
                    ptr.setNext(student);
                    found = true;  
                    break;
                }
                else if(studentToInsert.getHeight()<ptr.getNext().getStudent().getHeight())
                {
                    student.setNext(ptr.getNext());
                    ptr.setNext(student);
                    found = true;
                    break;
                }
    ...
    public void seatMusicalChairsWinner () 
    {
        if(musicalChairs.getNext()==musicalChairs && musicalChairs!=null)
        {
            ...
            for(int row = 0; row<studentsSitting.length; row++)
            {
                for(int col = 0; col<studentsSitting[row].length; col++)
                {
                    if(studentsSitting[row][col]==null && seatingLocation[row][col]==true)
                    {
                        studentsSitting[row][col] = stud;
